...
% Source task sampling rates
sourceSamplingRates = [0.5, 0.6, 0.7, 0.8, 0.9];

% Initialize results structure
if isfile('experiment_results_different_source_sampling_rates.mat')
    load('experiment_results_different_source_sampling_rates.mat', 'results');
else
    results = struct();
end
...
for dataIdx = 1:length(dataSet)
    ...
    for srIdx = 1:length(sourceSamplingRates)
        sourceSR = sourceSamplingRates(srIdx);
        fprintf('Processing %s, Source Task Sampling Rate = %.2f\n', dataFile, sourceSR);
        ...
        dataCfg.SRTgt = 0.05;                     % Sampling rate for target task
        dataCfg.vSRSrc = sourceSR * ones(KSource, 1); % Sampling rate for each source task
        ...
        algCfg.gammaFuse1 = 1e3;          % Learning rate for fusion
        algCfg.vAlpha = alpha * ones(KSource, 1);       % Alpha parameter
        algCfg.lambdaTil = 1e-2;         % Adjust regularization parameter
        algCfg.maxIter = 500;            % Adjust maximum number of iterations
        algCfg.maxIter_D_refine = 500;            % Adjust maximum number of iterations
        ...
        % Store results
        results.(strrep(dataFile, '.mat', '')).(['SR_' num2str(sourceSR*100)]) = struct();
        results.(strrep(dataFile, '.mat', '')).(['SR_' num2str(sourceSR*100)]).TensorSync_step1 = memo_TensorSync_step1;
        results.(strrep(dataFile, '.mat', '')).(['SR_' num2str(sourceSR*100)]).TensorSync_step2 = memo_TensorSync_step2;
        results.(strrep(dataFile, '.mat', '')).(['SR_' num2str(sourceSR*100)]).D_TensorSync = memo_D_TensorSync;
        results.(strrep(dataFile, '.mat', '')).(['SR_' num2str(sourceSR*100)]).TargetOnly = memo_TargetOnly;
        results.(strrep(dataFile, '.mat', '')).(['SR_' num2str(sourceSR*100)]).WtGT.tWtgt = tWtgt;
        results.(strrep(dataFile, '.mat', '')).(['SR_' num2str(sourceSR*100)]).Wobs.Wobs = targetObj.tY;
    end
end
